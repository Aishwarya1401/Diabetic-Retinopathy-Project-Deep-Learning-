...
cwd = os.getcwd()
print(cwd)
loadpath = cwd + "123.npz"
l = np.load(loadpath)
# See what's in here
print (l.files)

# Parse data
trainimg = l['train_dataset']
trainlabel = l['train_labels']
testimg = l['test_dataset']
testlabel = l['test_labels']
validimg = l['valid_dataset']
validlabel = l['valid_labels']
imgsize = [128,128]
use_gray = False
...
for epoch in range(training_epochs):
    ...
    print ("in epoch number :")
    print(epoch)  
    for i in range(num_batch): 
        randidx = np.random.randint(ntrain, size=batch_size)
        batch_xs = trainimg[randidx, :]
        batch_xs = np.reshape(batch_xs, (-1, 128))
    ...
    if epoch % display_step == 0 or epoch == training_epochs-1:
        ...
        testimg = np.reshape(batch_xs, (-1, 128))
